package com.qdauth.api.streams;
 ...
 *
 * <p>The Node.js original attempts JSON.parse on every binary frame and silently ignores parse
 * failures. This implementation mirrors that behaviour: text frames are parsed as JSON control
 * messages; binary frames are treated as media data. Spring's WebSocket layer distinguishes them at
 * the protocol level, so no try/catch parse attempt on binary data is required.
 */
